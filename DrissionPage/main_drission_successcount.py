...
NUM_BROWSERS = SLOT_NUM  # 슬롯 개수만큼 브라우저 생성
...
def _inc_success(slot_index: int) -> int:
    """성공 시 전체 카운터 증가 후 이합을 반환."""
    global _success_total
    with _success_lock:
        _success_total += 1
        total = _success_total
    print(f"[Main] 🎉 ✅ 전체 성공 누적 = {total} (last_success_slot={slot_index})")
    return total
...
# [1] 프로그램 시작 시 등 한 번만 호출되도록 메인 진입점에 넣어주세요
...
def monitor_service_optimized(url, proxy, slot_index, stop_event, r):
    ...
    try:
        ...
        if slot_index < len(SLOT_POSITIONS):
            window_position = SLOT_POSITIONS[slot_index]
            print(f"[Slot-{slot_index}] 📍 브라우저 위치: x={window_position['x']}, y={window_position['y']}")
        ...
        browser_wrapper = StealthMobileBrowser(
            ...
            window_position=window_position  # ✅ 위치 정보 전달
        )
