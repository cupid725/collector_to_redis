...
#TARGET_URL = "https://www.youtube.com/shorts/FmN9aknzeOE?feature=share"
NUM_BROWSERS = 3 
MOBILE_DEVICES_LIST = [
...
def monitor_service(url, proxy_url, index, stop_event, r):
    ...
    should_close_after_special_behavior = False  # ✅ 추가: 특별 행동 후 종료 플래그
            ...
            try:
                ...
                if status:
                    ...
                    if should_do_special:
                        ...
                        # ✅ 수정: simulate_mobile_behavior에 검색 키워드 전달
                        should_close_after_special_behavior = simulate_mobile_behavior(page, is_near_end=True, search_keyword=search_keyword)
                        special_behavior_done = True
                        
                        # ✅ 수정: 특별 행동 후 종료 플래그가 True이면 루프 탈출
                        if should_close_after_special_behavior:
                            print(f"   [Bot-{index}] 🏁 특별 행동 완료 후 종료 예정 (남은 시간: {watch_duration - elapsed:.0f}초)")
                            break
                else:
...
def main():
    ...
    print(f"🔍 검색 키워드: {len(SEARCH_KEYWORDS)}개 (랜덤 선택)")
